...
Public MustInherit Class FrmNotaReturPembelianTanpaTTB
    ...
    Protected Sub Cetak() Handles _Toolbar1_Button6.ItemClick
        ShowDevexpressReport(ReportPreview.KeyReport.Retur_Pembelian_Tanpa_TTB, TxtIDTransaksi.Text)
    ...
    Private Sub REBEdit_Click(sender As Object, e As System.EventArgs) Handles ReBEdit.ButtonClick
        If GridView1.FocusedColumn.FieldName = "Kode Barang" Then
            ...
            'CEK DATA ADA / TIDAK 
            Using dt_cari = SelectCon.execute("SELECT TOP 1 A.ID,A.KODE,A.NAMA, A.SAT_DIST1 AS SATUAN, A.QTY_KOLI AS ISI ,B.ISI AS KONV,C.HARGA_BARU HARGA_DIST FROM BARANG A INNER JOIN V_SATUAN_TO_PCS B ON A.ID=B.ID AND B.SATUAN1= A.SAT_DIST1 INNER JOIN VI_PRICE_LIST C ON A.ID=C.ID_BARANG LEFT JOIN LINK_BARANG_DIVISI D ON A.ID=D.ID_BARANG WHERE (A.ID='" & col("Kode Barang") & "' OR A.KODE='" & col("Kode Barang") & "') AND C.JENIS='Langganan' AND C.ID='UMUM' AND A.STATUS_AKTIF=1 AND D.KODE_DIVISI='" & TxtDivisi.Text & "' ORDER BY TGL_PRICE DESC")
...
CariData:
            ...
            Else
                ...
                End If
            End If
            Using dt_cari = SelectCon.execute("SELECT TOP 1 A.ID,A.KODE,A.NAMA, A.SAT_DIST1 AS SATUAN, A.QTY_KOLI AS ISI ,B.ISI AS KONV,C.HARGA_BARU HARGA_DIST FROM BARANG A INNER JOIN V_SATUAN_TO_PCS B ON A.ID=B.ID AND B.SATUAN1= A.SAT_DIST1 INNER JOIN VI_PRICE_LIST C ON A.ID=C.ID_BARANG LEFT JOIN LINK_BARANG_DIVISI D ON A.ID=D.ID_BARANG WHERE A.ID='" & col("Kode Barang") & "' AND C.JENIS='Langganan' AND C.ID='UMUM' AND A.STATUS_AKTIF=1 AND D.KODE_DIVISI='" & TxtDivisi.Text & "' ORDER BY TGL_PRICE DESC")
                If dt_cari.Rows.Count > 0 Then
                    col("No.") = GridView1.FocusedRowHandle + 1
                    col("ID Barang") = dt_cari.Rows(0).Item("ID")
                    col("Kode Barang") = dt_cari.Rows(0).Item("KODE")
...
Public Class InputNotaReturPembelianTanpaTTB
    Inherits FrmNotaReturPembelianTanpaTTB
    ...
    Private Sub InputPreDOSuper_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        Text = "Input Nota Retur Pembelian Tanpa TTB"
    ...
    Private Sub Simpan() Handles _Toolbar1_Button1.ItemClick
        ...
        Using SQLServer As New SQLServerTransaction
            ...
            'Detail
            If SQLServer.InsertDetail(dt.Select("[Kode Barang]<>''").CopyToDataTable, {ToObject(TxtIDTransaksi.Text), ToObject(TxtNoTransaksi.Text), ToSyntax("NULL"), ToSyntax("NULL"), "ID Barang", "Isi", ToSyntax("NULL"), "Koli", "Kuantum", "Satuan", "Konv", "Pieces", "Harga Satuan", "Disc %", "Disc Satuan", "No."}, "DETAIL_RETUR_PEMBELIAN") = False Then Exit Sub
...
Public Class EditNotaReturPembelianTanpaTTB
    Inherits FrmNotaReturPembelianTanpaTTB

    Private Sub EditPO_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        Text = "Nota Retur Pembelian Tanpa TTB"
        Status_Edit = True
        RDJenisPPN.Enabled = False
        HakForm("", "Retur", "Retur Pembellian Tanpa TTB", _Toolbar1_Button4, _Toolbar1_Button5, _Toolbar1_Button6)
    End Sub
