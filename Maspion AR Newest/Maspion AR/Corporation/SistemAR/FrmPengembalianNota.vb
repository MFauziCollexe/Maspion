...
Public MustInherit Class FrmPengembalianNota
    ...
    Private Sub FrmDeliveryOrder_Load(sender As Object, e As System.EventArgs) Handles MyBase.Load
        ...
        InitGrid.AddColumnGrid("No. Penyerahan", TypeCode.String, 50, False)
    ...
    Private Sub TambahNota()
        If Not Empty({TxtKodeCollector}) Then
            If dt.Select("[Nota]='" & TxtCariNota.Text & "'").Count = 0 Then
                Using dtCari = SelectCon.execute("SELECT * FROM V_AR_DSR_T_PENYERAHAN WHERE ID_NOTA='" & TxtCariNota.Text & "' OR NO_NOTA='" & TxtCariNota.Text & "'")
                    If dtCari.Rows.Count > 0 Then
                        If dtCari.Rows(0).Item("ST") = 1 Then
                            If dtCari.Rows(0).Item("COLLECTOR") = TxtKodeCollector.Text Then
                                If dtCari.Rows(0).Item("PG") = 0 Or dtCari.Rows(0).Item("ID_PENGEMBALIAN") = TxtIDTransaksi.Text Then
                                    Dim dr = dt.NewRow
                                    ...
                                    dr.Item("No. Penyerahan") = dtCari.Rows(0).Item("NO_PENYERAHAN")
                                    ...
                                    Urutin()
                                Else
                                    MessageBox.Show("Nota sudah dibuatkan pengembalian dengan nomor `" & dtCari.Rows(0).Item("NO_PENGEMBALIAN") & "`!", "Informasi", MessageBoxButtons.OK, MessageBoxIcon.Information)
                                End If
                            Else
                                MessageBox.Show("Nota ini bukan diserahkan pada `" & TxtNamaCollector.Text & "`!", "Informasi", MessageBoxButtons.OK, MessageBoxIcon.Information)
                            End If
                        Else
                            MessageBox.Show("Nota belum dibuatkan penyerahan!", "Informasi", MessageBoxButtons.OK, MessageBoxIcon.Information)
                        End If
                    Else
...
Public Class InputPengembalianNota
    Inherits FrmPengembalianNota

    Private Sub InputValidasiDSRPrw_Load(sender As Object, e As EventArgs) Handles Me.Load
        Text = "Input Pengembalian Nota"
    ...
    Private Function Generate() As Boolean
        Dim urut As Short
        Using dtGenerate = SelectCon.execute("SELECT NO_PENGEMBALIAN FROM AR_PENGEMBALIAN_NOTA WHERE NO_PENGEMBALIAN Like 'PG-" & Format(TglTransaksi.EditValue, "yyMM") & "-%' ORDER BY NO_PENGEMBALIAN DESC")
            If dtGenerate.Rows.Count = 0 Then
                urut = 1
            Else
                urut = Microsoft.VisualBasic.Right(dtGenerate.Rows(0).Item(0), 6) + 1
            End If
            TxtNoTransaksi.Text = "PG-" & Format(TglTransaksi.EditValue, "yyMM") & "-" & Format(urut, "000000")
        End Using

        Using dtGenerate = SelectCon.execute("SELECT ISNULL(MAX(CAST(REPLACE(ID_TRANSAKSI,'PG','') AS INT)),0) AS ID FROM AR_PENGEMBALIAN_NOTA")
    ...
    Private Sub Simpan() Handles _Toolbar1_Button1.ItemClick
        If Empty({TglTransaksi, TxtKodeCollector}) Then Exit Sub
        ...
        Using SQLServer As New SQLServerTransaction
            ...
            If SQLServer.InsertHeader({TxtIDTransaksi, TxtNoTransaksi, TglTransaksi, TxtKodeCollector, periode, UserID, ToSyntax("CURRENT_TIMESTAMP"), ToSyntax("NULL"), ToSyntax("NULL"), 0}, "AR_PENGEMBALIAN_NOTA") = False Then Exit Sub
            If SQLServer.InsertDetail(dt, {ToObject(TxtIDTransaksi.Text), ToObject(TxtNoTransaksi.Text), "Id Nota", "No."}, "AR_PENGEMBALIAN_NOTA_DETAIL") = False Then Exit Sub
...
Public Class EditPengembalianNota
    ...
    Private Sub GetData() Handles _Toolbar1_Button2.ItemClick, TxtIDTransaksi.EditValueChanged
        LoadData.GetData("SELECT NO_PENYERAHAN, TGL, COLLECTOR FROM AR_PENYERAHAN_NOTA WHERE ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'")
        LoadData.SetData({TxtNoTransaksi, TglTransaksi, TxtKodeCollector})

        LoadData.GetData("SELECT AA.URUTAN, A.ID_NOTA, A.NO_NOTA, A.NO_DO, A.NO_DSR, A.NO_VALIDASI, A.NO_PENYERAHAN, A.ID_CUSTOMER, B.NAMA, A.BRUTO, A.STD_DISC, A.ADD_DISC, 
        A.CASH_DISC, A.NETTO FROM AR_PENGEMBALIAN_NOTA_DETAIL AA JOIN V_AR_DSR_T_PENYERAHAN A ON A.ID_NOTA=AA.ID_NOTA JOIN CUSTOMER B ON A.ID_CUSTOMER=B.ID 
    ...
    Private Sub SimpanEdit() Handles _Toolbar1_Button1.ItemClick
        If Empty({TglTransaksi, TxtNoTransaksi, TxtKodeCollector}) Then Exit Sub
        ...
        Using SQLServer As New SQLServerTransaction
            SQLServer.InitTransaction()
            If SQLServer.UpdateHeader("NO_PENYERAHAN, TGL, COLLECTOR ,[MDUSER] ,[MDTIME]", {TxtNoTransaksi, TglTransaksi, TxtKodeCollector, ToSyntax(UserID), ToSyntax("CURRENT_TIMESTAMP")}, "AR_PENGEMBALIAN_NOTA", "ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'") = False Then Exit Sub
            If SQLServer.Delete("AR_PENGEMBALIAN_NOTA_DETAIL", "ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'") = False Then Exit Sub
            If SQLServer.InsertDetail(dt, {ToObject(TxtIDTransaksi.Text), ToObject(TxtNoTransaksi.Text), "Id Nota", "No."}, "AR_PENGEMBALIAN_NOTA_DETAIL") = False Then Exit Sub
            SQLServer.EndTransaction()
            Me.Dispose()
        End Using
    ...
    Private Sub Hapus() Handles _Toolbar1_Button5.ItemClick
        ...
        Using SQLServer As New SQLServerTransaction
            SQLServer.InitTransaction()
            If SQLServer.Delete("AR_PENGEMBALIAN_NOTA_DETAIL", "ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'") = False Then Exit Sub
            If SQLServer.Delete("AR_PENGEMBALIAN_NOTA", "ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'") = False Then Exit Sub
            SQLServer.EndTransaction()
            Me.Dispose()
        End Using
    End Sub
    ...
    Private Sub Batal() Handles _Toolbar1_Button4.ItemClick
        ...
        Using SQLServer As New SQLServerTransaction
            SQLServer.InitTransaction()
            If SQLServer.UpdateHeader("BATAL", {ToSyntax("1")}, "AR_PENGEMBALIAN_NOTA", "ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'") = False Then Exit Sub
