...
Public MustInherit Class FrmPelunasanRetur
    ...
    Private Sub FrmDeliveryOrder_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        InitGrid.AddColumnGrid("Harga Retur Pusat", TypeCode.Decimal, 100, False, DevExpress.Utils.FormatType.Numeric, "c2")
        InitGrid.AddColumnGrid("Disc %", TypeCode.Decimal, 60, False, DevExpress.Utils.FormatType.Numeric, , , ReCalc)
        InitGrid.AddColumnGrid("Disc Satuan", TypeCode.Decimal, 100, False, DevExpress.Utils.FormatType.Numeric, "c2", , ReCalc)
    ...
    Private Sub TxtNoTTB_ButtonClick(sender As Object, e As DevExpress.XtraEditors.Controls.ButtonPressedEventArgs) Handles TxtNoTTB.ButtonClick
        TxtIDTTB.Text = SearchTransaction(FrmPencarianTransaksi.KeyPencarian.Transaksi_Retur_Pusat)
    End Sub
    Private Sub TxtNoTTB_KeyPress(sender As Object, e As System.Windows.Forms.KeyPressEventArgs) Handles TxtNoTTB.KeyPress
        If CharKeypress(TxtNoTTB, e) Then TxtIDTTB.Text = SearchTransaction(FrmPencarianTransaksi.KeyPencarian.Transaksi_Retur_Pusat)
    End Sub
    Private Sub TxtIDTTB_EditValueChanged(sender As Object, e As System.EventArgs) Handles TxtIDTTB.EditValueChanged
        ...
        If Status_Edit = False Then
            LoadData.GetData("SELECT NO_RETUR_PUSAT,TGL_PENGAKUAN,DIVISI,KETERANGAN_CETAK,JENIS_RETUR,KODE_APPROVE,KODE_CUSTOMER,JENIS_PPN,[SUBTOTAL],[DISC_QTY],[DISC_QTY_NOMINAL],[DISC_REG],[DISC_REG_NOMINAL],[DISC_1],[DISC_1_NOMINAL],[DISC_2],[DISC_2_NOMINAL],[DISC_3],[DISC_3_NOMINAL],[CASH_DISC],[CASH_DISC_NOMINAL],[DISC_QTY1],[DISC_QTY_NOMINAL1],[DPP],[PPN],[BEBAS_PPN] FROM RETUR_PUSAT WHERE ID_TRANSAKSI='" & TxtIDTTB.Text & "'")
            LoadData.SetData({TxtNoTTB, TglTTB, TxtDivisi, txtKeterangan, RGJenis, TxtKodeApprove, TxtIDCustomer, RDJenisPPN, TxtSubTotal, TxtDiskonQty1, TxtDiskonQty1Nominal, TxtDiskonReguler, TxtDiskonRegulerNominal, TxtDiskon1, TxtDiskon1Nominal, TxtDiskon2, TxtDiskon2Nominal, TxtDiskon3, TxtDiskon3Nominal, TxtCashDiskon, TxtCashDiskonNominal, TxtDiskonQty2, TxtDiskonQty2Nominal, TxtDPP, TxtPPN, ChkBebasPPN})
            SplashScreenManager1.ShowWaitForm()
            LoadData.GetData("SELECT A.URUTAN,A.ID_BARANG,C.KODE,C.NAMA,A.SATUAN,A.ISI, A.KOLI,A.QUANTITY,A.PCS,A.KONVERSI,drp.HARGA,A.HARGA,A.DISC,A.DISKON_SATUAN,0 FROM (RETUR_PUSAT AA INNER JOIN DETAIL_RETUR_PUSAT A ON AA.ID_TRANSAKSI=A.ID_TRANSAKSI) LEFT JOIN V_D_RB_T_RP B ON AA.ID_NOTA_RETUR=B.ID_TRANSAKSI AND A.ID_BARANG=B.ID_BARANG LEFT JOIN BARANG C ON A.ID_BARANG=C.ID left join DETAIL_RETUR_PEMBELIAN DRP on drp.ID_TRANSAKSI = aa.ID_NOTA_RETUR and drp.ID_BARANG = a.ID_BARANG WHERE A.ID_TRANSAKSI='" & TxtIDTTB.Text & "' ORDER BY A.URUTAN")
...
Public Class InputPelunasanRetur
    Inherits FrmPelunasanRetur
    ...
    Private Function Generate() As Boolean
        Dim urut As Short
        Using dtGenerate = SelectCon.execute("SELECT NO_PELUNASAN_RETUR FROM AR_PELUNASAN_RETUR a join RETUR_PUSAT B on a.ID_RETUR_PUSAT = b.ID_TRANSAKSI WHERE NO_PELUNASAN_RETUR Like '" & TxtNamaDivisi.Text & "(" & InisialPerusahaan & ")" & "%' AND YEAR(a.TGL)=" & Format(TglTransaksi.EditValue, "yyyy") & " AND b.DIVISI='" & TxtDivisi.Text & "' ORDER BY NO_PELUNASAN_RETUR DESC")
        ...
        Using dtGenerate = SelectCon.execute("SELECT ISNULL(MAX(CAST(REPLACE(ID_TRANSAKSI,'PRB','') AS INT)),0) AS ID FROM AR_PELUNASAN_RETUR")
            TxtIDTransaksi.Text = "PRB" & CInt(dtGenerate.Rows(0).Item(0)) + 1
        End Using
        Return True
    End Function
    ...
    Private Sub Simpan() Handles _Toolbar1_Button1.ItemClick
        ...
        Using SQLServer As New SQLServerTransaction
            ...
            'Header
            If SQLServer.InsertHeader({TxtIDTransaksi, TxtNoTransaksi, TglPengakuan, TxtIDTTB, TxtNoTTB, periode, UserID, ToSyntax("CURRENT_TIMESTAMP"), ToSyntax("NULL"), ToSyntax("NULL"), 0}, "AR_PELUNASAN_RETUR") = False Then Exit Sub
            'Detail
            ...
            Dim nominalreturawal As Double = 0
            Dim subtotalreturawal As Double = 0
            Dim subtotalreturpusat As Double = 0
            Dim nominalreturpusat As Double = 0
            Dim noreturbeli As String = ""
            dt.AcceptChanges()
            For i = 0 To GridView1.RowCount - 1
                If IsDBNull(GridView1.GetDataRow(i)("Harga Satuan")) = True Then
                    subtotalreturawal += 0
                Else
                    subtotalreturawal += GridView1.GetDataRow(i)("Harga Satuan")
                End If
                If IsDBNull(GridView1.GetDataRow(i)("Harga Retur Pusat")) = True Then
                    subtotalreturpusat += 0
                Else
                    subtotalreturpusat += GridView1.GetDataRow(i)("Harga Retur Pusat")
                End If
            Next
            nominalreturpusat = TxtDPP.Value + TxtPPN.Value
            Using dtreturbeli = SelectCon.execute("select a.DPP + a.PPN,a.NO_RETUR from RETUR_PEMBELIAN a join RETUR_PUSAT B on a.ID_TRANSAKSI = b.ID_NOTA_RETUR where B.ID_TRANSAKSI = '" & TxtIDTTB.Text & "'")
                If dtreturbeli.Rows.Count > 0 Then
                    nominalreturawal = dtreturbeli.Rows(0)(0)
                    noreturbeli = dtreturbeli.Rows(0)(1)
                End If
            End Using
            Dim apiutangucfretur = Akun.PIUTANG_UCF_PRB
            Dim apersediaankantorpusatretur = Akun.PERSEDIAAN_PUSAT_PRB
            Dim aselisihlebihbesarPW = Akun.SELISIH_HARGA_PW_PRB
            Dim aselisihlebihbesarPusat = Akun.SELISIH_HARGA_PUSAT_PRB
            Dim ProsJurnal As New ProsesJurnal(ProsesJurnal.JenisJurnal.Jurnal_Memo, TglPengakuan.DateTime, "", TxtNoTransaksi.Text, "Jurnal Transaksi", "03", "", 999, 0, SQLServer, ProsesJurnal.StatusJurnal.Baru)

            ProsJurnal.AddJurnal(New DetailJurnal(PosisiJurnal.Debet, Setup.GetAkun(apiutangucfretur), "NOTA RETUR " & TxtNamaDivisi.Text & " NO." & Strings.Right(TxtNoTransaksi.Text, 6), nominalreturpusat))
            ProsJurnal.AddJurnal(New DetailJurnal(PosisiJurnal.Kredit, Setup.GetAkun(apersediaankantorpusatretur), "SJ KE SBY NO." & Strings.Right(noreturbeli, 6), nominalreturawal))
            If nominalreturpusat < nominalreturawal Then
                ProsJurnal.AddJurnal(New DetailJurnal(PosisiJurnal.Debet, Setup.GetAkun(aselisihlebihbesarPW), "SLH.HRG.SAT NOTA RETUR " & TxtNamaDivisi.Text & " NO." & Strings.Right(TxtNoTransaksi.Text, 6), nominalreturawal - nominalreturpusat))
            End If
            If nominalreturpusat > nominalreturawal Then
                ProsJurnal.AddJurnal(New DetailJurnal(PosisiJurnal.Kredit, Setup.GetAkun(aselisihlebihbesarPusat), "SLH.HRG.SAT NOTA RETUR " & TxtNamaDivisi.Text & " NO." & Strings.Right(TxtNoTransaksi.Text, 6), nominalreturpusat - nominalreturawal))
            End If
            ProsJurnal.Submit()

            Dim prosjurnalUCF As New ProsesJurnal(ProsesJurnal.JenisJurnal.Jurnal_Memo, TglPengakuan.DateTime, "", TxtNoTransaksi.Text, "Jurnal Transaksi UCF", "UCF", "", 999, 0, SQLServer, ProsesJurnal.StatusJurnal.Baru)
            Dim ahndipusatsby = Akun.HUTANG_PUSAT_PRB
            Dim apndiucfdiv = "DIVISI_UCF_PELUNASAN_RETUR" & TxtDivisi.Text

            Dim namasbu As String = ""
            Using dtsbu = SelectCon.execute("select b.NAMA from LINK_SBU_DIVISI a join SBU b on a.KODE_SBU = b.KODE where A.KODE_DIVISI = '" & TxtDivisi.Text & "'")
                If dtsbu.Rows.Count > 0 Then
                    namasbu = dtsbu.Rows(0)(0)
                End If
            End Using

            prosjurnalUCF.AddJurnal(New DetailJurnal(PosisiJurnal.Debet, Setup.GetAkun(apndiucfdiv), "PNDI " & namasbu & " (" & TxtNamaDivisi.Text & ")", nominalreturpusat))
            prosjurnalUCF.AddJurnal(New DetailJurnal(PosisiJurnal.Kredit, Setup.GetAkun(ahndipusatsby), "HNDI KP SBY", nominalreturpusat))
            prosjurnalUCF.Submit()
            Dim pt As String = ""
            Using dtcari = SelectCon.execute("select a.kode_pt from LINK_PT_SBU a join LINK_SBU_DIVISI b on a.KODE_SBU = b.KODE_SBU where b.KODE_DIVISI = '" & TxtDivisi.Text & "'")
                If dtcari.Rows.Count > 0 Then
                    pt = dtcari.Rows(0)(0)
                End If
            End Using
            Dim prosjurnalSBU As New ProsesJurnal(ProsesJurnal.JenisJurnal.Jurnal_Memo, TglPengakuan.DateTime, "", TxtNoTransaksi.Text, "Jurnal Transaksi SBU", pt, "", 999, 0, SQLServer, ProsesJurnal.StatusJurnal.Baru)
            Dim ahndiucfsbu = Akun.HUTANG_UCF_PRB
            Dim apdrsby = Akun.PIUTANG_RETUR_PUSAT_PRB
            prosjurnalSBU.AddJurnal(New DetailJurnal(PosisiJurnal.Debet, Setup.GetAkun(apdrsby), "NOTA RETUR SBY " & TxtNamaDivisi.Text & " NO." & Strings.Right(TxtNoTransaksi.Text, 6), nominalreturpusat))
            prosjurnalSBU.AddJurnal(New DetailJurnal(PosisiJurnal.Kredit, Setup.GetAkun(ahndiucfsbu), "UCF", nominalreturpusat))
            prosjurnalSBU.Submit()
...
Public Class EditPelunasanRetur
    ...
    Private Sub SimpanEdit() Handles _Toolbar1_Button1.ItemClick
        ...
        Using SQLServer As New SQLServerTransaction
            SQLServer.InitTransaction()
            'Header
            If SQLServer.UpdateHeader("[NO_PELUNASAN_RETUR] ,[TGL] ,[ID_RETUR_PUSAT] ,[NO_RETUR_PUSAT] ,[MDUSER] ,[MDTIME]", {TxtNoTransaksi, TglPengakuan, TxtIDTTB, TxtNoTTB, ToSyntax(UserID), ToSyntax("CURRENT_TIMESTAMP")}, "AR_PELUNASAN_RETUR", "ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'") = False Then Exit Sub
            'Proses Jurnal
            Dim Setup As New SetupAkun
            Dim nominalreturawal As Double = 0
            Dim subtotalreturawal As Double = 0
            ...
            Dim aselisihlebihbesarPW = Akun.SELISIH_HARGA_PW_PRB
            Dim aselisihlebihbesarPusat = Akun.SELISIH_HARGA_PUSAT_PRB
            Dim ProsJurnal As New ProsesJurnal(ProsesJurnal.JenisJurnal.Jurnal_Memo, TglPengakuan.DateTime, "", TxtNoTransaksi.Text, "Jurnal Transaksi", "03", "", 999, 0, SQLServer, ProsesJurnal.StatusJurnal.Edit)
            ...
            Dim prosjurnalUCF As New ProsesJurnal(ProsesJurnal.JenisJurnal.Jurnal_Memo, TglPengakuan.DateTime, "", TxtNoTransaksi.Text, "Jurnal Transaksi UCF", "UCF", "", 999, 0, SQLServer, ProsesJurnal.StatusJurnal.Edit)
            Dim ahndipusatsby = Akun.HUTANG_PUSAT_PRB
            Dim apndiucfdiv = "DIVISI_UCF_PELUNASAN_RETUR" & TxtDivisi.Text
            ...
            Using dtcari = SelectCon.execute("select a.kode_pt from LINK_PT_SBU a join LINK_SBU_DIVISI b on a.KODE_SBU = b.KODE_SBU where b.KODE_DIVISI = '" & TxtDivisi.Text & "'")
                ...
                End If
            End Using
            Dim prosjurnalSBU As New ProsesJurnal(ProsesJurnal.JenisJurnal.Jurnal_Memo, TglPengakuan.DateTime, "", TxtNoTransaksi.Text, "Jurnal Transaksi SBU", pt, "", 999, 0, SQLServer, ProsesJurnal.StatusJurnal.Edit)
            Dim ahndiucfsbu = Akun.HUTANG_UCF_PRB
            Dim apdrsby = Akun.PIUTANG_RETUR_PUSAT_PRB
            prosjurnalSBU.AddJurnal(New DetailJurnal(PosisiJurnal.Debet, Setup.GetAkun(apdrsby), "NOTA RETUR SBY " & TxtNamaDivisi.Text & " NO." & Strings.Right(TxtNoTransaksi.Text, 6), nominalreturpusat))
            prosjurnalSBU.AddJurnal(New DetailJurnal(PosisiJurnal.Kredit, Setup.GetAkun(ahndiucfsbu), "UCF", nominalreturpusat))
    ...
    Private Sub Hapus() Handles _Toolbar1_Button5.ItemClick
        ...
        Using SQLServer As New SQLServerTransaction
            ...
            If SQLServer.Delete("AR_PELUNASAN_RETUR", "ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'") = False Then Exit Sub
            Dim pt As String = ""
            Using dtcari = SelectCon.execute("select a.kode_pt from LINK_PT_SBU a join LINK_SBU_DIVISI b on a.KODE_SBU = b.KODE_SBU where b.KODE_DIVISI = '" & TxtDivisi.Text & "'")
                If dtcari.Rows.Count > 0 Then
                    pt = dtcari.Rows(0)(0)
                End If
            End Using
            Dim ProsJurnal As New ProsesJurnal(ProsesJurnal.JenisJurnal.Jurnal_Memo, TglPengakuan.DateTime, "", TxtNoTransaksi.Text, "Jurnal Transaksi", "03", "", 999, 0, SQLServer, ProsesJurnal.StatusJurnal.Hapus)
            Dim prosjurnalUCF As New ProsesJurnal(ProsesJurnal.JenisJurnal.Jurnal_Memo, TglPengakuan.DateTime, "", TxtNoTransaksi.Text, "Jurnal Transaksi UCF", "UCF", "", 999, 0, SQLServer, ProsesJurnal.StatusJurnal.Hapus)
            Dim prosjurnalSBU As New ProsesJurnal(ProsesJurnal.JenisJurnal.Jurnal_Memo, TglPengakuan.DateTime, "", TxtNoTransaksi.Text, "Jurnal Transaksi SBU", pt, "", 999, 0, SQLServer, ProsesJurnal.StatusJurnal.Hapus)
    ...
    Private Sub Batal() Handles _Toolbar1_Button4.ItemClick
        ...
        Using SQLServer As New SQLServerTransaction
            SQLServer.InitTransaction()
            ' If SQLServer.Delete("SALDO_STOK", "ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'") = False Then Exit Sub
            If SQLServer.UpdateHeader("BATAL", {ToSyntax("1")}, "AR_PELUNASAN_RETUR", "ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'") = False Then Exit Sub
            Dim pt As String = ""
            Using dtcari = SelectCon.execute("select a.kode_pt from LINK_PT_SBU a join LINK_SBU_DIVISI b on a.KODE_SBU = b.KODE_SBU where b.KODE_DIVISI = '" & TxtDivisi.Text & "'")
                If dtcari.Rows.Count > 0 Then
                    pt = dtcari.Rows(0)(0)
                End If
            End Using
            Dim ProsJurnal As New ProsesJurnal(ProsesJurnal.JenisJurnal.Jurnal_Memo, TglPengakuan.DateTime, "", TxtNoTransaksi.Text, "Jurnal Transaksi", "03", "", 999, 0, SQLServer, ProsesJurnal.StatusJurnal.Batal)
            Dim prosjurnalUCF As New ProsesJurnal(ProsesJurnal.JenisJurnal.Jurnal_Memo, TglPengakuan.DateTime, "", TxtNoTransaksi.Text, "Jurnal Transaksi", "UCF", "", 999, 0, SQLServer, ProsesJurnal.StatusJurnal.Batal)
            Dim prosjurnalSBU As New ProsesJurnal(ProsesJurnal.JenisJurnal.Jurnal_Memo, TglPengakuan.DateTime, "", TxtNoTransaksi.Text, "Jurnal Transaksi", pt, "", 999, 0, SQLServer, ProsesJurnal.StatusJurnal.Batal)
