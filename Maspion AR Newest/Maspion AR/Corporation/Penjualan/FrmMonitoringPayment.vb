...
Public MustInherit Class FrmMonitoringPayment
...
#Region "Shared Sub"
    ...
    Private Sub Simpan()
        ...
        Using SQLServer As New SQLServerTransaction
            ...
            If SQLServer.InsertDetail(dt.Select("Checklist=1").CopyToDataTable, {"ID DO", "ID Customer", "No. DO", "Total", "Tgl. Payment", "Bayar", ToObject(UserID), ToSyntax("CURRENT_TIMESTAMP")}, "MONITORING_PAYMENT") = False Then Exit Sub
    ...
    Private Sub HapusPayment()
        ...
        Using SQLServer As New SQLServerTransaction
            SQLServer.InitTransaction()
            If SQLServer.InsertDetail(dt.Select("Checklist=1").CopyToDataTable, {"ID DO", "ID Customer", "No. DO", "Total", "Tgl. Payment", ToNegative("Bayar"), ToObject(UserID), ToSyntax("CURRENT_TIMESTAMP")}, "MONITORING_PAYMENT") = False Then Exit Sub
            SQLServer.EndTransaction()
            Proses(1)
            RDStatus.SelectedIndex = 0
            GetData()
...
#End Region
    ...
    Protected Sub GetData()
        If RDStatus.SelectedIndex = 0 Then
            dt = SelectCon.execute("SELECT CAST(0 AS BIT) AS Checklist,A.ID_TRANSAKSI AS [ID DO],A.NO_DO AS [No. DO],A.TGL AS [Tgl. Transaksi],A.TGL_PENGAKUAN AS [Tgl. Pengakuan],B.ID AS [ID Customer],B.NAMA AS [Nama Customer],DPP+PPN AS Total,A.KETERANGAN_CETAK AS Keterangan ,A.KETERANGAN_INTERNAL AS [Keterangan Internal],CAST(NULL AS DATE) AS [Tgl. Payment],DPP+PPN AS Bayar FROM DELIVERY_ORDER A LEFT JOIN CUSTOMER B ON A.KODE_CUSTOMER=B.ID LEFT JOIN VI_PAY_KONTAN C ON A.ID_TRANSAKSI=C.ID_TRANSAKSI WHERE PEMBAYARAN='Kontan' AND C.STATUS_LUNAS=0 AND A.BATAL<>1 AND BAGIAN='" & EnumDescription(Bagian) & "'")
        ElseIf RDStatus.SelectedIndex = 1 Then
            dt = SelectCon.execute("SELECT CAST(0 AS BIT) AS Checklist,A.ID_TRANSAKSI AS [ID DO],A.NO_DO AS [No. DO],A.TGL AS [Tgl. Transaksi],A.TGL_PENGAKUAN AS [Tgl. Pengakuan],B.ID AS [ID Customer],B.NAMA AS [Nama Customer],DPP+PPN AS Total,A.KETERANGAN_CETAK AS Keterangan ,A.KETERANGAN_INTERNAL AS [Keterangan Internal],CAST(C.TGL AS DATE) AS [Tgl. Payment],DPP+PPN AS Bayar FROM DELIVERY_ORDER A LEFT JOIN CUSTOMER B ON A.KODE_CUSTOMER=B.ID LEFT JOIN VI_PAY_KONTAN C ON A.ID_TRANSAKSI=C.ID_TRANSAKSI WHERE PEMBAYARAN='Kontan' AND C.STATUS_LUNAS=1 AND A.BATAL<>1 AND BAGIAN='" & EnumDescription(Bagian) & "' AND A.PERIODE='" & Format(TglPeriode.Value, "MMyy") & "'")
        ElseIf RDStatus.SelectedIndex = 2 Then
            dt = SelectCon.execute("SELECT CAST(0 AS BIT) AS Checklist,A.ID_TRANSAKSI AS [ID DO],A.NO_DO AS [No. DO],A.TGL AS [Tgl. Transaksi],A.TGL_PENGAKUAN AS [Tgl. Pengakuan],B.ID AS [ID Customer],B.NAMA AS [Nama Customer],DPP+PPN AS Total,A.KETERANGAN_CETAK AS Keterangan ,A.KETERANGAN_INTERNAL AS [Keterangan Internal],CAST(C.TGL AS DATE) AS [Tgl. Payment],DPP+PPN AS Bayar FROM DELIVERY_ORDER A LEFT JOIN CUSTOMER B ON A.KODE_CUSTOMER=B.ID LEFT JOIN VI_PAY_KONTAN C ON A.ID_TRANSAKSI=C.ID_TRANSAKSI WHERE PEMBAYARAN='Kontan' AND A.BATAL<>1 AND BAGIAN='" & EnumDescription(Bagian) & "' AND A.PERIODE='" & Format(TglPeriode.Value, "MMyy") & "'")
        End If
        ...
        Try
            ...
            GridView1.Columns(10).Width = 20
            GridView1.Columns(10).OptionsColumn.AllowFocus = False
            GridView1.Columns(11).Width = 30
            GridView1.Columns(11).OptionsColumn.AllowFocus = False
            GridView1.Columns(11).Visible = False
            GridView1.BestFitColumns()
            GridView1.Columns(2).BestFit()
            GroupBox1.Text = "[ " & GridView1.RowCount & " Data Delivery Order ] "

            With GridView1.Columns.Item(7).SummaryItem
                .DisplayFormat = "{0:n2}"
                .FieldName = "Total"
