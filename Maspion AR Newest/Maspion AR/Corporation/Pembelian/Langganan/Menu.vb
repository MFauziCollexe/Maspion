Public Class MenuPOLangganan
    ...
    Private Sub MenuPO_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        Key = "Purchase Order Langganan"
        AddHandler Activated, AddressOf GetData
        AddHandler GridView1.FocusedRowChanged, AddressOf GetDataDetail
        HakMenu("Pembelian", "Langganan", "Purchase Order", BarButtonBaru, {BtnEdit, BarButtonEdit}, BtnCetak)
    End Sub

    Private Sub Baru() Handles BarButtonBaru.ItemClick
        InputPOLangganan.MdiParent = Me.MdiParent
        InputPOLangganan.Show()
        InputPOLangganan.Focus()
    End Sub

    Private Sub Edit() Handles BarButtonEdit.ItemClick
        If GridView1.RowCount > 0 Then
            If Not IsDBNull(GridView1.GetFocusedRow(0)) Then
                EditPOLangganan.MdiParent = Me.MdiParent
                EditPOLangganan.Show()
                EditPOLangganan.Focus()
                EditPOLangganan.TxtIDTransaksi.Text = GridView1.GetFocusedRow(0)
            End If
        End If
    End Sub
    ...
    Private Sub BtnLihat_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles BtnLihat.ItemClick
        If GridView1.RowCount > 0 Then
            If Not IsDBNull(GridView1.GetFocusedRow(0)) Then
                Dim frm As New EditPOLangganan
...
Public Class MenuNotaClaimLangganan
    Inherits FrmMenuDetail

    Private Sub GetData()
        TBMenu.DataSource = SelectCon.execute("SELECT A.ID_TRANSAKSI,A.NO_CLAIM,A.TGL_PENGAKUAN,A.NO_NOTA,A.TGL_NOTA,B.NAMA,C.NAMA_USER,DPP+PPN AS TOTAL,A.BATAL FROM CLAIM A LEFT JOIN DIVISI B ON A.DIVISI=B.KODE LEFT JOIN USERS C ON A.CRUSER=C.ID_USER LEFT JOIN LINK_USER_DIVISI D ON A.DIVISI=D.KODE_DIVISI WHERE PERIODE='" & periode & "' AND A.BAGIAN LIKE '%Langganan%' AND D.ID_USER='" & UserID & "'")
        TBMenu.Refresh()
        Frame1.Text = "[ " & GridView1.DataRowCount & " Data Purchase Order ]"
        GridView1.Columns(0).Caption = "ID Transaksi"
        GridView1.Columns(0).Width = 40
    ...
    Private Sub MenuPO_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        Key = "Nota Claim Pembelian Langganan"
        AddHandler Activated, AddressOf GetData
        AddHandler GridView1.FocusedRowChanged, AddressOf GetDataDetail
        HakMenu("Pembelian", "Langganan", "Claim Pembelian", BarButtonBaru, {BtnEdit, BarButtonEdit}, BtnCetak)
    End Sub

    Private Sub Baru() Handles BarButtonBaru.ItemClick
        InputClaimPembelianLangganan.MdiParent = Me.MdiParent
        InputClaimPembelianLangganan.Show()
        InputClaimPembelianLangganan.Focus()
    End Sub

    Private Sub Edit() Handles BarButtonEdit.ItemClick
        If GridView1.RowCount > 0 Then
            If Not IsDBNull(GridView1.GetFocusedRow(0)) Then
                EditClaimPembelianLangganan.MdiParent = Me.MdiParent
                EditClaimPembelianLangganan.Show()
                EditClaimPembelianLangganan.Focus()
                EditClaimPembelianLangganan.TxtIDTransaksi.Text = GridView1.GetFocusedRow(0)
            End If
        End If
    End Sub
    ...
    Private Sub BtnLihat_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles BtnLihat.ItemClick
        If GridView1.RowCount > 0 Then
            If Not IsDBNull(GridView1.GetFocusedRow(0)) Then
                Dim frm As New EditClaimPembelianLangganan
                frm.MdiParent = Me.MdiParent
                frm.Show()
                frm.Focus()
                frm.Text = Replace(frm.Text, "Edit", "Lihat")
...
Public Class MenuNotaPembelianLangganan
    Inherits FrmMenuDetail

    Private Sub GetData()
        TBMenu.DataSource = SelectCon.execute("SELECT A.ID_TRANSAKSI,A.NO_NOTA,A.TGL_PENGAKUAN,A.NO_NOTA_PENJUALAN, A.NO_PO,A.TGL_PO,B.NAMA,C.NAMA_USER,A.DPP+A.PPN AS TOTAL,A.BATAL FROM PEMBELIAN A LEFT JOIN SBU B ON A.KODE_SUPPLIER=B.KODE LEFT JOIN USERS C ON A.CRUSER=C.ID_USER LEFT JOIN LINK_USER_DIVISI D ON A.DIVISI=D.KODE_DIVISI WHERE BAGIAN LIKE '%Langganan%' AND D.ID_USER='" & UserID & "' AND PERIODE='" & periode & "' ORDER BY TGL")
        TBMenu.Refresh()
    ...
    Private Sub MenuPO_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        Key = "Nota Pembelian Langganan"
        AddHandler Activated, AddressOf GetData
        AddHandler GridView1.FocusedRowChanged, AddressOf GetDataDetail
        HakMenu("Pembelian", "Langganan", "Nota Pembelian", BarButtonBaru, {BtnEdit, BarButtonEdit}, BtnCetak)
    End Sub

    Private Sub Baru() Handles BarButtonBaru.ItemClick
        InputNotaPembelianLangganan.MdiParent = Me.MdiParent
        InputNotaPembelianLangganan.Show()
        InputNotaPembelianLangganan.Focus()
    End Sub

    Private Sub Edit() Handles BarButtonEdit.ItemClick
        If GridView1.RowCount > 0 Then
            If Not IsDBNull(GridView1.GetFocusedRow(0)) Then
                EditNotaPembelianLangganan.MdiParent = Me.MdiParent
                EditNotaPembelianLangganan.Show()
                EditNotaPembelianLangganan.Focus()
                EditNotaPembelianLangganan.TxtIDTransaksi.Text = GridView1.GetFocusedRow(0)
            End If
        End If
    End Sub
    ...
    Private Sub BtnLihat_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles BtnLihat.ItemClick
        If GridView1.RowCount > 0 Then
            If Not IsDBNull(GridView1.GetFocusedRow(0)) Then
                Dim frm As New EditNotaPembelianLangganan
                frm.MdiParent = Me.MdiParent
                frm.Show()
                frm.Focus()
                frm.Text = Replace(frm.Text, "Edit", "Lihat")
