...
#Region "Shared Sub"
    ...
    Protected Sub Generate()
        Dim urut As Short
        Using dtGenerate = SelectCon.execute("SELECT NO_TRANSAKSI FROM PRICE_LIST WHERE NO_TRANSAKSI Like 'PL/" & Format(TglTransaksi.DateTime, "yyMM") & "%' AND YEAR(TGL_PRICE)=" & Format(TglPengakuan.EditValue, "yyyy") & " ORDER BY NO_TRANSAKSI DESC")
    ...
    Protected Sub Simpan()
        ...
        Using SQLServer As New SQLServerTransaction
            ...
            'Header
            If SQLServer.InsertHeader({TxtIDTransaksi, TxtNoTransaksi, TglTransaksi, TglPengakuan, TxtDivisi, RDJenisPriceList, ToSyntax("NULL"), ToSyntax("NULL"), txtKeterangan, txtKeteranganInternal, periode, UserID, ToSyntax("CURRENT_TIMESTAMP"), ToSyntax("NULL"), ToSyntax("NULL"), 0, ConcatCustomer()}, "PRICE_LIST") = False Then Exit Sub
            'Detail
            If SQLServer.InsertDetail(dt.Select("[ID Barang]<>''").CopyToDataTable, {ToObject(TxtIDTransaksi.Text), ToObject(TxtNoTransaksi.Text), "ID Barang", "Satuan Besar", "Satuan Besar2", "Isi", "Satuan Kecil", "Satuan Kecil2", "Harga Lama", "Harga Baru", ToObject(RDJenis.EditValue), "No."}, "DETAIL_PRICE_LIST") = False Then Exit Sub
            Try
                If SQLServer.InsertDetail(dtCustomer.Select("[ID Customer]<>''").CopyToDataTable, {ToObject(TxtIDTransaksi.Text), "ID Customer", "Kode Customer"}, "DETAIL_PRICE_LIST_CUSTOMER") = False Then Exit Sub
    ...
    Protected Sub GetData()
        If TxtIDTransaksi.Text <> "" Then
            LoadData.GetData("SELECT A.[NO_TRANSAKSI] ,A.[TGL] ,A.[TGL_PRICE] ,A.[DIVISI] ,A.[JENIS_PRICE] ,A.[KETERANGAN_CETAK] ,A.[KETERANGAN_INTERNAL],B.[JENIS] FROM PRICE_LIST A INNER JOIN DETAIL_PRICE_LIST B ON A.ID_TRANSAKSI=B.ID_TRANSAKSI WHERE A.ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'")
            LoadData.SetData({TxtNoTransaksi, TglTransaksi, TglPengakuan, TxtDivisi, RDJenisPriceList, txtKeterangan, txtKeteranganInternal, RDJenis})

            LoadData.GetData("SELECT A.URUTAN,A.ID_BARANG,B.KODE,B.NAMA,A.SATUAN,A.SATUAN1,A.ISI,A.SATUANK,A.SATUANK1,A.HARGA_LAMA,A.HARGA_BARU FROM DETAIL_PRICE_LIST A LEFT JOIN BARANG B ON A.ID_BARANG=B.ID WHERE A.ID_TRANSAKSI='" & TxtIDTransaksi.Text & "' ORDER BY A.URUTAN")
            LoadData.SetDataDetail(dt, True)
            LoadData.GetData("SELECT B.ID,B.KODE_APPROVE,A.NAMA FROM CUSTOMER A INNER JOIN DETAIL_PRICE_LIST_CUSTOMER B ON A.ID=B.ID WHERE B.ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'")
    ...
    Protected Sub SimpanEdit()
        ...
        Using SQLServer As New SQLServerTransaction
            SQLServer.InitTransaction()
            'Header
            If SQLServer.UpdateHeader("[NO_TRANSAKSI] ,[TGL] ,[TGL_PRICE] ,[DIVISI] ,[JENIS_PRICE] ,[KETERANGAN_CETAK] ,[KETERANGAN_INTERNAL] ,[MDUSER] ,[MDTIME], [CUSTOMER]", {TxtNoTransaksi, TglTransaksi, TglPengakuan, TxtDivisi, RDJenisPriceList, txtKeterangan, txtKeteranganInternal, ToSyntax(UserID), ToSyntax("CURRENT_TIMESTAMP"), ConcatCustomer()}, "PRICE_LIST", "ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'") = False Then Exit Sub
            'Detail
            If SQLServer.Delete("DETAIL_PRICE_LIST", "ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'") = False Then Exit Sub
            If SQLServer.Delete("DETAIL_PRICE_LIST_CUSTOMER", "ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'") = False Then Exit Sub
            If SQLServer.InsertDetail(dt.Select("[ID Barang]<>''").CopyToDataTable, {ToObject(TxtIDTransaksi.Text), ToObject(TxtNoTransaksi.Text), "ID Barang", "Satuan Besar", "Satuan Besar2", "Isi", "Satuan Kecil", "Satuan Kecil2", "Harga Lama", "Harga Baru", ToObject(RDJenis.EditValue), "No."}, "DETAIL_PRICE_LIST") = False Then Exit Sub
            Try
                If SQLServer.InsertDetail(dtCustomer.Select("[ID Customer]<>''").CopyToDataTable, {ToObject(TxtIDTransaksi.Text), "ID Customer", "Kode Customer"}, "DETAIL_PRICE_LIST_CUSTOMER") = False Then Exit Sub
            Catch ex As Exception
    ...
    Protected Sub Hapus()
        ...
        If TglPengakuan.DateTime <= Now.Date Then
...
#End Region
    ...
    Private Sub FrmDeliveryOrder_Load(sender As Object, e As System.EventArgs) Handles Me.Load
        ...
        InitGrid.AddColumnGrid("Harga Baru", TypeCode.Decimal, 50, True, DevExpress.Utils.FormatType.Numeric, "n2", , ReCalc)
...
CariData:
    ...
    Private Sub _Toolbar1_Button7_ItemClick(sender As System.Object, e As DevExpress.XtraBars.ItemClickEventArgs) Handles _Toolbar1_Button7.ItemClick
        Dim frm As New InputPriceList
        With frm
            .TglTransaksi.DateTime = Now
            .TglPengakuan.DateTime = Now
