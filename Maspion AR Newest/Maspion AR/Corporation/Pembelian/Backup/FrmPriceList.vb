...
#Region "Shared Sub"
    ...
    Protected Sub Simpan()
        ...
        Using SQLServer As New SQLServerTransaction
            ...
            'Detail
            If SQLServer.InsertDetail(dt.Select("[Kode Barang]<>''").CopyToDataTable, {ToObject(TxtIDTransaksi.Text), ToObject(TxtNoTransaksi.Text), "ID Barang", "Satuan Besar", "Satuan Besar2", "Isi", "Satuan Kecil", "Satuan Kecil2", "Harga Lama", "Harga Baru", ToObject(RDJenis.EditValue), "No."}, "DETAIL_PRICE_LIST") = False Then Exit Sub
    ...
    Protected Sub GetData()
        If TxtIDTransaksi.Text <> "" Then
            LoadData.GetData("SELECT A.[NO_TRANSAKSI] ,A.[TGL] ,A.[TGL_PRICE] ,A.[DIVISI] ,A.[JENIS_PRICE] ,A.[KODE_CUSTOMER] ,A.[KODE_APPROVE] ,A.[KETERANGAN_CETAK] ,A.[KETERANGAN_INTERNAL],B.[JENIS] FROM PRICE_LIST A INNER JOIN DETAIL_PRICE_LIST B ON A.ID_TRANSAKSI=B.ID_TRANSAKSI WHERE A.ID_TRANSAKSI='" & TxtIDTransaksi.Text & "'")
            LoadData.SetData({TxtNoTransaksi, TglTransaksi, TglPengakuan, TxtDivisi, RDJenisPriceList, TxtIDCustomer, TxtKodeApprove, txtKeterangan, txtKeteranganInternal, RDJenis})
...
#End Region
    ...
    Private Sub ReBEdit_ButtonClick(sender As Object, e As System.EventArgs) Handles ReBEdit.ButtonClick
        If GridView1.FocusedColumn.FieldName = "Kode Barang" Then
            ...
            If RDJenis.SelectedIndex = 0 Then : satuan = "A.SAT_DIST1, A.SAT_DIST2, A.QTY_DIST, A.HARGA_DIST"
            Else : satuan = "A.SAT_SUPER1 AS SAT_DIST1, A.SAT_SUPER2 AS SAT_DIST2,1 AS QTY_DIST, A.HARGA_SUPER AS HARGA_DIST" : End If
            Dim col As DataRow = GridView1.GetFocusedDataRow()
            'CEK DATA ADA / TIDAK 
            Using dt_cari = SelectCon.execute("SELECT A.ID, A.KODE, A.NAMA, " & satuan & ", A.SAT_SUPER1, A.SAT_SUPER2 FROM BARANG A WHERE A.ID='" & col("Kode Barang") & "' OR A.KODE='" & col("Kode Barang") & "'")
...
CariData:
            ...
            Else
                ...
                End If
            End If
            Using dt_cari = SelectCon.execute("SELECT A.ID, A.KODE, A.NAMA, " & satuan & ", A.SAT_SUPER1, A.SAT_SUPER2, A.HARGA_DIST,A.HARGA_DIST FROM BARANG A WHERE A.ID='" & col("ID Barang") & "'")
                If dt_cari.Rows.Count > 0 Then
                    col("No.") = GridView1.FocusedRowHandle + 1
                    col("ID Barang") = dt_cari.Rows(0).Item("ID")
                    col("Kode Barang") = dt_cari.Rows(0).Item("KODE")
